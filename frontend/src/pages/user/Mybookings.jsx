...
function Mybookings() {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      ...
      try {
        ...
        if (!user || !user.id) {
          setError("Foydalanuvchi ma'lumotlari topilmadi. Iltimos, qaytadan kiring.");
          toast.error("Foydalanuvchi ma'lumotlari topilmadi. Iltimos, qaytadan kiring.");
          setLoading(false);
          return;
        }
        ...
        const response = await axios.get(`http://localhost:4000/user/get-user-booking/${userId}`, { // Changed id to userId
          headers: {
        ...
        const errorMsg = err.response?.data?.message || "Bronlarni yuklashda server xatoligi yuz berdi.";
        setError(errorMsg);
    ...
    return (
      <div className="flex justify-center items-center min-h-screen bg-gray-100">
        <div className="text-center">
          ...
          <p className="mt-4 text-lg font-semibold text-pink-700">Bronlar yuklanmoqda...</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="flex justify-center items-center min-h-screen bg-gray-100">
        <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative max-w-md mx-auto text-center" role="alert">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-rose-50 to-white p-4 md:p-8">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl md:text-4xl font-bold text-pink-700 mb-8 text-center">Mening Bronlarim</h1>
                ...
                <div className="md:flex">
                  <div className="p-6 flex-grow">
                    <div className="flex justify-between items-start">
                      <h2 className="text-2xl font-bold text-pink-700 mb-2">{booking.venue_name || "Noma'lum To'yxona"}</h2>
                      <span className={`px-3 py-1 text-sm font-semibold rounded-full ${
                        booking.status === 'confirmed' ? 'bg-green-100 text-green-700' :
                        booking.status === 'pending' ? 'bg-yellow-100 text-yellow-700' :
                        booking.status === 'cancelled' ? 'bg-red-100 text-red-700' :
                        booking.status === 'endi bo`ladigan' ? 'bg-blue-100 text-blue-700' : // Added specific status
                        'bg-gray-100 text-gray-700'
                      }`}>
                        {booking.status === 'confirmed' ? "Tasdiqlangan" :
                         booking.status === 'pending' ? "Kutilmoqda" :
                         booking.status === 'cancelled' ? "Bekor qilingan" :
                         booking.status === 'endi bo`ladigan' ? "Kelgusi" : // Added specific status text
                         booking.status?.toUpperCase()}
                      </span>
                    </div>
                    
                    <p className="text-gray-600 mb-1">
                      <span className="font-semibold">Mijoz:</span> {booking.firstname} {booking.lastname}
                    </p>
                    <p className="text-gray-600 mb-1">
                      <span className="font-semibold">Telefon:</span> {booking.client_phone}
                    </p>
                    <p className="text-gray-600 mb-1">
                      <span className="font-semibold">Hudud:</span> {booking.district_name || "Noma'lum"}
                    </p>
                    <p className="text-gray-600 mb-1">
                      <span className="font-semibold">Sana:</span> {new Date(booking.reservation_date).toLocaleDateString('uz-UZ', { year: 'numeric', month: 'long', day: 'numeric' })}
                    </p>
                    <p className="text-gray-600 mb-1">
                      <span className="font-semibold">Vaqt:</span> {new Date(booking.reservation_date).toLocaleTimeString('uz-UZ', { hour: '2-digit', minute: '2-digit' })}
                    </p>
                    <p className="text-gray-600 mb-3">
                      <span className="font-semibold">Odam soni:</span> {booking.guest_count || "Noma'lum"}
                    </p>
